	...
	<div class="wrapper">
		...
		<div style="margin-top: 20px;">
			...
			<table class="inlineTable pull-right margin-top-60px size">
				...
				<tr>
					<td>Tanggal</td>
					<td>:</td>
					<td>{{$head->ip_tanggal}}</td>
				</tr>
				<tr>
					<td>Jatuh Tempo</td>
					<td>:</td>
					<td>{{$head->ip_jatuh_tempo}}</td>
				</tr>
				<tr>
					<td>Kode.Cust.</td>
					<td>:</td>
					<td>{{$head->ip_kode_customer}}</td>
				</tr>
			</table>
		...
		<div class="pembungkus">
			<table class="size textcenter  table-responsive" width="100%">
				...
				<tr>
					...
					<td class="textleft borderbottomtabel borderrighttabel"> {{$row->ipd_nomor_do}} </td>
					<td class="borderbottomtabel borderrighttabel"> {{$row->ipd_tgl_do}} </td>
					<td class="textleft borderbottomtabel borderrighttabel">{{$row->ipd_keterangan}} </td>
					<td class="borderbottomtabel borderrighttabel">{{$row->ipd_kuantum}}&nbsp;{{$row->ipd_kode_satuan}}</td>
					<td class="textright borderbottomtabel borderrighttabel"> {{ number_format($row->ipd_harga_satuan, 2, ",", ".") }} </td>
					<td class="textright borderbottomtabel borderrighttabel"> {{ number_format($row->ipd_harga_bruto, 2, ",", ".") }} </td>
					<td class="textright borderbottomtabel borderrighttabel"> {{ number_format($row->ipd_harga_netto, 2, ",", ".") }} </td>
				</tr>
				@endforeach
				@foreach($push as $a => $val)
				<tr>
					<td  height="25" style="text-align:right;border: none" class="textcenter bot right top"></td>
					<td style="text-align:right;border: none"  class="bot right"> </td>
					<td style="text-align:right;border: none" class="bot right">  </td>
					<td style="text-align:left;border: none" class="bot right">  </td>
					<td style="text-align:right;border: none" class="bot right"> </td>
					<td style="text-align:right;border: none" class="bot right"> </td>
					<td style="text-align:left;border: none" class="bot right">  </td>
					<td style="text-align:right;border: none" class="bot right"> </td>
		...
		<table class="size" width="100%">
			<tr>
				...
				<td class="textright borderrighttabel borderbottomtabel bordertoptabel">{{ number_format($head->ip_netto, 2, ",", ".") }}</td>
			</tr>
		</table>
		</table>
		<table width="100%" class="border size hiddenbordertop table-responsive">
			<tr>
				...
				<td class="borderrighttabel borderbottomtabel textright">{{ number_format($head->ip_diskon+$head->ip_diskon2, 2, ",", ".") }}</td>
			</tr>
			<tr>
				...
				<td class="borderrighttabel borderbottomtabel textright">{{ number_format($head->ip_netto-$head->ip_diskon-$head->ip_diskon2, 2, ",", ".") }}</td>
			</tr>
			<tr>
				<td class="borderrighttabel textleft" width="26%">BCA KCP Bhayangkara Surabaya</td>
				<td class="textleft borderrighttabel" width="26%">&nbsp;</td>
				<td class="textright borderrighttabel" width="26.2%">&nbsp;</td>
				@if($head->ip_ppntpe == 'npkp')
				...
				@endif
				<td class="borderrighttabel borderbottomtabel textright">{{ number_format($head->ip_ppnrp, 2, ",", ".") }}</td>
			...
			<tr>
				...
				<td class="borderrighttabel borderbottomtabel textright">{{ number_format($head->ip_pajak_lain, 2, ",", ".") }}</td>
			</tr>
			<tr>

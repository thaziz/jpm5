	...
	<div class="wrapper">
		...
		<div style="margin-top:20px;">
			<table class="inlineTable pull-right margin-top-60px size">
				...
				@foreach ($rute as $row)
				<tr>
					<td class="top bot right left textcenter paddingkota">{{$row->kota}}</td>
		...
		<div class="pembungkus">
			<table class="size textleft" width="100%">
				...
				<tr>
					<td height="27" class="textright bot right">{{$i++}}.</td>
					<td style="display: none;">{{$jml=$jml+$row->jumlah}}.</td>
					<td class="textleft bot right"> {{$row->nomor_do}} </td>
					<td class="textleft bot right"> {{$row->tanggal}} </td>
					<td class="textleft bot right"> {{$row->nama_pengirim}} </td>

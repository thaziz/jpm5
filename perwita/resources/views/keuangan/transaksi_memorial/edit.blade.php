...
<script>
  $(document).ready(function(){
    ...
    $('#debet_first').val('{{ ( $data_detail[0]->jrdt_statusdk == 'D' ) ? $data_detail[0]->jrdt_value : 0 }}');
    $('#kredit_first').val('{{ ( $data_detail[0]->jrdt_statusdk == 'K' ) ? $data_detail[0]->jrdt_value : 0 }}');
    initiate_coa_lawan();
    ...
    $('#simpan').click(function(evt){
      ...
      if(validate_form()){
        $.ajax(baseUrl+"/keuangan/transaksi_memorial/update",{
          type: "post",
          timeout: 15000,

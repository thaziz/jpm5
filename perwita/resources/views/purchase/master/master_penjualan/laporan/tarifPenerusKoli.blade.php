                ...
                <div class="box-body">
                  ...
                  <table id="addColumn" class="table table-bordered table-striped tbl-item">
                    ...
                    <tbody>
                      ...
                      <tr> 
                        <td>{{ $index+1 }}</td>
                        <td><input type="hidden" name="" value="{{$val->id_tarif_koli}}">{{$val->id_tarif_koli}}</td>
                        <td>{{$val->prov}}</td>
                        <td>{{$val->kot}}</td>
                        <td align="center">{{$val->kec}}</td>
                        <td align="right">{{"Rp " . number_format($val->h1,0,",",".")}}</td>
                        <td align="right">{{"Rp " . number_format($val->h2,0,",",".")}}</td>
                        <td align="right">{{"Rp " . number_format($val->h3,0,",",".")}}</td>
                        <td align="right">{{"Rp " . number_format($val->h4,0,",",".")}}</td>
                        <td align="right">{{"Rp " . number_format($val->h5,0,",",".")}}</td>
                        <td align="right">{{"Rp " . number_format($val->h6,0,",",".")}}</td>
                        <td align="right">{{"Rp " . number_format($val->h7,0,",",".")}}</td>
                        <td align="right">{{"Rp " . number_format($val->h8,0,",",".")}}</td>
                      </tr>
                      @endforeach
                    </tbody>       
                  </table>
         ...
         buttons: [
            {
                 ...
                 /* messageTop: 'Hasil pencarian dari Nama : ',*/
                  text: ' Excel',
                  className:'excel',
                  title:'LAPORAN TARIF PENERUS KOLI',
                  filename:'PENERUS KOLI-'+a+b+c,
   ...
   function filterColumn () {
    ...
    $('.select-picker1').change(function(){
      ...
      $.ajax({
        ...
        url: baseUrl + '/reportpeneruskoli/reportpeneruskoli',
        type: "post",    

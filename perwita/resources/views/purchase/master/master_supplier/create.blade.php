   ...
   clearInterval(reset);
    ...
    $('#tmbh_data_barang').click(function(){
      ...
      var rowBrg = "<tr id=dataitem item-"+$no+">" +
                    "<td> <b>" + $no +"</b> </td>" +               
                    "<td> <select class='form-control chosen-select' name='idbarang[]'>  @foreach($data['item'] as $item) <option value={{$item->kode_item}}> {{$item->nama_masteritem}} </option> @endforeach </select>" +

        ...
        <div class="col-sm-6">
          <table class="table table_header table-bordered table-striped">
            ...
            <tr>
              <td>No Transaksi</td>
              <td>
                <input readonly="" value="{{$data->bkk_nota}}" class="form-control" type="text" name="no_trans">
              </td>
            </tr>
            <tr>
              <td>Tanggal</td>
              <td><input readonly="" class="form-control" value="{{$data->bkk_tgl}}" type="text" name="tanggal"></td>
            </tr>
            <tr>
              <td>Jenis Bayar</td>
              <td>
                  <input readonly="" class="form-control kode_bayar" style="width: 20%; display: inline-block;" value="" type="text" name="kode_bayar">     
                  <select onchange="jenis_bayar()" disabled="" class="jenis_bayar  form-control" style="width: 79%; display: inline-block;">
                    <option>- Pilih Jenis Bayar -</option>
                    @foreach($jenisbayar as $val)
                    @if($val->idjenisbayar == $data->bkk_jenisbayar)
            ...
            <tr>
              <td>Supplier</td>
                <td>
                  @if($data->bkk_jenisbayar == 2 || $data->bkk_jenisbayar == 6 || $data->bkk_jenisbayar == 7 || $data->bkk_jenisbayar == 9 || $data->bkk_jenisbayar == 3)
                  <div class="">
                    <input readonly="" class="form-control code_supplier" value="{{$data->kode_agen}}" style="width: 30%; display: inline-block;">
                         
                    <input readonly=""  class="form-control  nama_supplier" style="width: 69%; display: inline-block;" value="{{$data->nama_agen}}" type="text" name="nama_faktur"> 
                  </div> 
                  @else
                  <div class="untuk_patty" >
                    ...
                    <input readonly="" value="{{$data->bkk_supplier}}" class="form-control nama_orang" style="width: 69%; display: inline-block;" type="text" name="nama_orang"> 
                  </div>
                  @endif                       
              </td>
            </tr>
            <tr>
              <td>Keterangan</td>
              <td>
                  <input  class="form-control keterangan" style="width: 100%; display: inline-block;"  type="text" value="{{$data->bkk_keterangan}}" name="keterangan">                        
              </td>
            </tr>
          </table>
        </div>
            ...
            <tr>
              ...
              <td>
                <div class="input-group">
                  ...
                  <select disabled="" name="nama_akun" class="form-control nama_akun chosen-select-width1" onchange="jenis_bayar()" style="width: 100% !important;">
                        <option value="0">- Pilih-Akun -</option>
                        @foreach($akun_kas as $val)
                          @if($val->id_akun == $data->bkk_akun_kas)
  ...
  <div class="ibox-content col-sm-12 ">
   <div class="col-sm-6 " >
    ...
    <td class="form-inline">
      ...
      <select name="nama_akun" class="form-control nama_akun_biaya chosen-select-width7" style="width: 74% !important;">
            <option value="0">- Pilih-Akun -</option>
            @foreach($akun as $val)
            <option value="{{$val->id_akun}}">{{$val->id_akun}} - {{$val->nama_akun}}</option>
            @endforeach 
    ...
    <td >
      ...
      <button style="margin-left: 5px;" type="button" class="btn btn-warning pull-right "  onclick="printing()"><i class="fa fa-print">&nbsp;print</i></button>
 ...
 <div class=" ibox-content col-sm-12 tb_sb_hidden" >
  ...
  <hr>
      ...
      <tbody align="center" class="">
        ...
        <tr>
          <td>
            <label class="seq_pat_text">{{$i+1}}</label>
            <input type="hidden" class="pat_sub pat_seq_{{$i+1}}"  value="{{$i+1}}" >
          </td>
          <td width="400" align="left">
            <label class="keterangan_text">{{$val->bkkd_keterangan}}</label>
            <input type="hidden" class="ket_patty"  name="ket_patty[]" value="{{$val->bkkd_keterangan}}" >
          </td>
          <td>
            <label class="acc_text">{{$val->bkkd_akun}}</label>
          ...
          <td>
            ...
            <input type="hidden" class="debit_patty_table" name="debit_patty[]" value="{{$val->bkkd_debit}}" >
          </td>
          <td>
            <a class="btn btn-primary btn-xs fa fa-pencil" align="center" onclick="edit_patty(this)" title="Edit"></i></a>
            <a class="btn btn-danger fa fa-trash btn-xs"  align="center" onclick="hapus_patty(this)" title="hapus"></i></a>
          </td>
        </tr>
        @endforeach
      </tbody>    
...
<div class="tab-content">
  ...
  <div class="col-sm-4 " >
    ...
    <td >
      ...
      <input readonly="" type="hidden" class="form-control edit_bkkd_id"  >
    </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td >
        ...
        <input type="hidden" class="old_lunas">
      </td>
    </tr>
    <tr>
 ...
 <div class=" ibox-content col-sm-12 tb_sb_hidden" >
  ...
  <hr>
      <table class="table table-bordered table-hover tabel_faktur" style="font-size: 12px">
      <thead align="center">
        <tr>
        <th width="30">No</th>
        <th >No Faktur</th>
      ...
      <tbody align="center" class="">
        @foreach($data_dt as $i => $val)
          @if(isset($val->fp_netto))

          <tr>
            <td>
              <label class="seq_fak_text">{{$i+1}}</label>
              <input type="hidden" class="fak_sub fak_seq{{$i+1}}"  value="{{$i+1}}" >
              <input type="hidden" class="bkkd_id" name="bkkd_id[]"  value="{{$val->bkkd_id}}" >
            </td>
            <td>
              <label class="nofaktur_text">{{$val->bkkd_ref}}</label>
              <input type="hidden"  class="nofaktur"  name="nofaktur[]" value="{{$val->bkkd_ref}}" >
            </td>
            <td>
              <label>{{$data->bkk_tgl}}</label>
            </td>
            <td><label>{{210111000}}</label></td> 
            <td><label>{{number_format($val->fp_netto,2,',','.')}}</label></td>
            <td width="50">
              <input style="width: 130px;text-align: right;" readonly="" type="text" class="pelunasan_detail form-control" name="pelunasan[]" value="{{number_format($val->bkkd_total,2,',','.')}}">
              <input style="width: 130px"  type="hidden" class="sisa_terbayar_detail form-control"  value="">
            </td>
            <td>
              <input type="text" class="keterangan_bkkd form-control" name="keterangan_bkkd[]" value="{{$val->bkkd_keterangan}}">
            </td>
            <td>
              <a onclick="hapus_faktur_edit(this)" type="button" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i></a>
            </td>
          </tr>
          @elseif(isset($val->v_hasil))
          <tr>
            <td>
              <label class="seq_fak_text">{{$i+1}}</label>
              <input type="hidden" class="fak_sub fak_seq{{$i+1}}"  value="{{$i+1}}" >
            ...
            <td><label>{{210111000}}</label></td> 
            <td><label>{{number_format($val->v_hasil,2,',','.')}}</label></td>
            <td width="50">
              <input style="width: 130px;text-align: right;" readonly="" type="text" class="pelunasan_detail form-control" name="pelunasan[]" value="{{number_format($val->bkkd_total,2,',','.')}}">
              <input style="width: 130px"  type="hidden" class="sisa_terbayar_detail form-control"  value="">
            </td>
      ...
      <button type="button" class="btn btn-primary pull-right save_faktur disabled"  id="save_faktur" onclick="save_faktur()" data-dismiss="modal">Simpan Data</button>
  </div>
</div>
<!-- END FORM FAKTUR -->
<!-- FORM BODY UANG MUKA -->
 ...
 <div class=" ibox-content col-sm-12 tb_sb_hidden" >
  ...
  <hr>
      ...
      <tbody align="center" class="">
        ...
        <tr>
          <td>
            <label class="seq_um_text">{{$i+1}}</label>
            <input type="hidden" class="um_sub um_seq_{{$i+1}}"  value="{{$i+1}}" >
            <input type="hidden" class="um_id " name="bkkd_id[]"  value="{{$val->bkkd_id}}">
          </td>
          <td>
            <label class="no_um_text">{{$val->bkkd_ref}}</label>
            <input type="hidden" class="no_um_table"  name="no_um[]" value="{{$val->bkkd_ref}}" >
          </td>
          <td>
            <label class="supp_um_text"> {{$val->bkkd_supplier}}</label>
            <input type="hidden" class="supp_um_table"  name="supp_um_table[]" value=" {{$val->bkkd_supplier}}" >
          </td>
          <td>
            <label class="ket_um_text">{{$val->bkkd_keterangan}}</label>
            <input type="hidden" class="ket_um_table"  name="ket_um[]" value="{{$val->bkkd_keterangan}}" >
          </td>
          <td align="right">
            <label class="ket_um_text">{{number_format($val->um_jumlah,2,',','.')}}</label>
            <input type="hidden" name="total_um[]" class="total_um_table form-control" value="{{number_format($val->um_jumlah,2,',','.')}}" readonly="">
          </td>
          <td align="right">
            <label class="sisa_um_text">{{number_format($val->um_sisapelunasan+$val->bkkd_total,2,',','.')}}</label>
            <input type="hidden" class="sisa_um_table form-control" value="{{number_format($val->um_sisapelunasan+$val->bkkd_total,2,',','.')}}" readonly="">
          </td>
          <td>
            <input style="text-align: right;" type="text" name="nominal_um[]" class="nominal_um_table form-control" value="{{number_format($val->bkkd_total,2,',','.')}}" >
          </td>
          <td>
  ...
  <div class="modal-dialog">
    ...
    <td width="200" class="form-inline">
      <input type="text" name="kode_akun" class="form-control kode_akun_patty_update" style="width: 70px;" value="0" readonly="">
       <select name="nama_akun" class="form-control nama_akun_patty_update chosen-select-width" style="width: 74% !important;">
            <option value="0">- Pilih-Akun -</option>
            @foreach($akun as $val)
...
@foreach($data_dt as $i=>$val)
  patty_array.push('{{$val->bkkd_akun}}');
  var temp2 = parseFloat('{{$val->bkkd_total}}');
  temp2 = Math.round(temp2).toFixed(0);
  temp2 = parseInt(temp2);
  total_pembayaran.push(temp2);
@endforeach
var temp=0;
...
var patty  = $('.tabel_patty_cash').DataTable({
                  ...
                  'ordering' :false,
                  "columnDefs": [
                    { className: "kiri", "targets": [ 1 ] },
                    { className: "kanan", "targets": [ 3 ] }
                  ]
              })
...
var config1 = {
             ...
             for (var selector in config7) {
               $(selector).chosen(config7[selector]);
             }   
...
function save_pat(){
  ...
  swal({
    ...
    text: "Update Data Bukti Kas Keluar!",
    type: "warning",
...
function validate_simpan_faktur(){
  var total = $('.total_pembayaran').val();
  total     = total.replace(/[^0-9\-]+/g,"");
  total     = total/100;
  // console.log(tabel_faktur.page.info().recordsTotal);
  if (tabel_faktur.page.info().recordsTotal == 0 && total == 0) {
...
$(document).ready(function(){
  @foreach($data_dt as $i => $val)
  @if (isset($val->fp_idfaktur)) {
     id.push('{{$val->fp_idfaktur}}');
   }
   @elseif(isset($val->v_hasil)) 
     id.push('{{$val->v_id}}');
   @endif
...
function hapus_faktur_edit(p){

  var par   = p.parentNode.parentNode;
  var id_data    = $(par).find('.bkkd_id').val();
  ...
  function(){
      ...
      $.ajax({
        url:baseUrl+'/buktikaskeluar/hapus_data_faktur',
        data: 'id='+id_data,
      success:function(response){
        var par = p.parentNode.parentNode;
        ...
        id.splice(indexing,1);
            ...
            $('.bayar_faktur').val("");
            $('.total_faktur').val("");
            $('.sisa_bayar_faktur').val("");
            $('.total_pembayaran_faktur').val("");
            $('.sisa_terbayar_faktur').val("");
            $('.sisa_faktur').val("");
            $('.sisa_bayar_faktur').val("");
            validate_simpan_faktur();
...
// $(par_dt).find('.tot').html(Math.round(pelunasan).toFixed(2));
// MERUBAH TOTAL
...
function save_faktur(){
  ...
  function(){
      ...
      $.ajax({
        url:baseUrl+'/buktikaskeluar/update_faktur',
        data: $('.table_header :input').serialize()
        +'&'+$('.table_jurnal :input').serialize()
...
$('.nominal_um_table').blur(function(){

  var par         = $(this).parents('tr');
  var total_um    = $(par).find('.sisa_um_table').val();
   ...
   if (index == -1) {
    ...
    um_array.push(no_um);
     tabel_um.row.add( [
                    ...
                    +'<input type="hidden" class="um_id " name="bkkd_id[]"  value="'+0+'">',
                    ...
                    '<label class="sisa_um_text">'+sisa_um+'</label>'+'<input type="hidden" class="sisa_um_table" value="'+sisa_um+'" >',
...
function hapus_um(p){
  ...
  var id_data   = $(par).find('.um_id').val();
  console.log(id);
      ...
      success:function(response){
            ...
            title: "Berhasil!",
                   ...
                   showConfirmButton: true
                       ...
                       $('.nominal_um_table').addClass('disabled');

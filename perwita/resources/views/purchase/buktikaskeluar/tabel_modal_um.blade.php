...
<table class="table tabel_modal table-hover">
  ...
  <tbody>
  	...
  	<tr onclick="pilih_faktur(this)" style="cursor: pointer;">
  		...
  		<td class="no_faktur">{{ $val->um_nomorbukti }}</td>
  		<td>{{ carbon\carbon::parse($val->um_tgl)->format('d/m/Y') }}</td>
  		<td>{{ carbon\carbon::parse($val->um_tgl)->format('d/m/Y') }}</td>
  		<td align="right">{{ number_format($val->um_jumlah, 0, ",", ".") }}</td>
  		<td align="right">{{ number_format($val->um_sisapelunasan, 0, ",", ".") }}</td>
  		<td align="center"> - </td>
  		<td align="center">

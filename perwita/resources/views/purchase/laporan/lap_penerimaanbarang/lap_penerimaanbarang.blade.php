               ...
               {{--  Tanggal : 01 July 2017 s/d 31 July 2017 --}}
                  ...
                  <table class="table table-bordered datatable table-striped">
                      ...
                      <tr>                                                     
                        <th> Gudang : </th> 
                          <td> 
                                <input id="gudang" type="text" class="form-control ">
                          </td>  
                          <th> Terima dari : </th> 
                            <td> 
                                <input id="terima" type="text" class="form-control" >
                            </td>
                      </tr>
                      <tr>                                                     
                        <th> Akun Hutang Dagang : </th> 
                          <td colspan="4"> 
                                <input id="pb_acchutangdagang" type="text" class="form-control ">
                          </td>  
                      </tr>
                      <tr>
                        <th> Dimulai : </th> 
                  ...
                  <table id="addColumn" class="table table_header table-bordered table-striped"> 
                    ...
                    <tr>
                      ...
                      <td>{{ $index+1 }} </td>
                      <td>  {{ $element->pb_lpb }}  </td>
                      <td>{{ $element->pb_suratjalan }}  </td>
                      <td>{{ $element->supplier }} </td>
                      <td>{{ $element->pb_terimadari }} </td>
                      <td>{{ $element->gudang }}  </td>
                      <td>{{ $element->akun }} </td>
...
<script type="text/javascript">
  ...
  function filterColumn ( ) {
    ...
    $('#gudang').on( 'keyup', function () {
         table.column(6).search( this.value ).draw();
      });
    $('#terima').on( 'keyup', function () {
         table.column(5).search( this.value ).draw();
      });
    $('#pb_acchutangdagang').on( 'keyup', function () {
    ...
    function cetak(){
     ...
     var asw=[];
      ...
      $.ajax({
        ...
        url: baseUrl + '/reportpenerimaanbarang/reportpenerimaanbarang',
         type: "post",

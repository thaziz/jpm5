                    ...
                    <table class="table table-striped table-bordered table-hover">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="ed_total_debet" readonly="readonly" class="form-control" style="text-transform: uppercase; text-align: right" value="{{ $data->total_kredit or 0 }}" >
                                </td>
                            </tr>
                            <tr>
                                <td style="width:120px; padding-top: 0.4cm">Total Kredit</td>
                                <td>
                                    <input type="text" name="ed_total_kredit" readonly="readonly" class="form-control" style="text-transform: uppercase; text-align: right" value="{{ $data->total_debet or 0 }}" >
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <h4 class="modal-title">Insert Edit Item jurnal_umum</h4>
                        </div>
                        <div class="modal-body">
                            <form class="form-horizontal detail">
                                <table id="table_data" class="table table-striped table-bordered table-hover">
                                    <tbody>
                                        <tr>
                                          <td style="width:120px; padding-top: 0.4cm">Akun</td>
                                          <td>
                                                <select class="chosen-select-width" name="cb_akun" id="cb_akun" >
                                                    <option></option>
                                                @foreach ($akun as $row)
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="text" class="form-control" name="ed_debet" style="text-align:right"></td>                      
                                        </tr>
                                        <tr>
                                            <td style="padding-top: 0.4cm">Kredit</td>
                                            <td><input type="text" class="form-control" name="ed_kredit" style="text-align:right"></td>                      
                                        </tr>
                                    </tbody>
                                </table>
                            </form>
...
<script type="text/javascript">
    $(document).ready( function () {
        ...
        $('#table_data').DataTable({
            ...
            "retrieve" : true,
            "ajax": {
                "url": baseUrl + "/keuangan/jurnal_umumform/tabel_data_detail",
                "type": "GET",
            ...
            { "data": "akun" },
            { "data": "memo" },
            { "data": "debet" , render: $.fn.dataTable.render.number( '.'),"sClass": "cssright" },
            { "data": "kredit" , render: $.fn.dataTable.render.number( '.'),"sClass": "cssright" },
            { "data": "button" },
            ]
        });
        var config = {
    ...
    $(document).on("click","#btnadd",function(){
        ...
        $("#modal").modal("show");
        $("#cb_akun").trigger('chosen:activate');
        $.ajax(
        {
            url :  baseUrl + "/keuangan/jurnal_umumform/save_data",
            type: "POST",
            dataType:"JSON",
    ...
    $(document).on( "click",".btnedit", function() {
        ...
        $.ajax(
        {
            ...
            success: function(data, textStatus, jqXHR)
            {
                ...
                $("select[name='cb_akun']").val(data.kode_akun).trigger('chosen:updated');
    ...
    $(document).on("click","#btnsave",function(){
        $.ajax(
        {
            url : baseUrl + "/keuangan/jurnal_umumform/save_data_detail",
            type: "POST",
            dataType:"JSON",
            ...
            success: function(data, textStatus, jqXHR)
            {
                if(data.crud == 'N'){
                    if(data.result == 1){
                        ...
                        $("#modal").modal('hide');
                        $("input[name='ed_nomor']").attr('readonly','readonly');
                        $("input[name='ed_total_debet']").val(data.total[0].total_debet);
                        $("input[name='ed_total_kredit']").val(data.total[0].total_kredit);
                        $("#btn_add").focus();
                    }else{
                        alert("Gagal menyimpan data!");
                    }
    ...
    $(document).on( "click",".btndelete", function() {
        ...
        $.ajax({
            type: "POST",
            url : baseUrl + "/keuangan/jurnal_umumform/hapus_data_detail",
            dataType:"JSON",
            data: value,
            success: function(data, textStatus, jqXHR)
            {

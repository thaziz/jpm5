                  ...
                  <table id="table_data" class="table table-bordered table-striped">
                    <thead>
                        <tr>
                            <th> Kode</th>
                            <th> Nama </th>
                            <th> Posisi di Buku Besar </th>
                            <th> Posisi di Laba Rugi </th>
                            <th> Aksi </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <form class="form-horizontal  kirim">
                          <table id="table_data" class="table table-striped table-bordered table-hover">
                            <tbody>
                                ...
                                <tr>
                                    <td style="padding-top: 0.4cm">Posisi Buku Besar</td>
                                    <td>
                                        <select  class="form-control" name="cb_posisi_di_buku_besar">
                                ...
                                <tr>
                                    <td style="padding-top: 0.4cm">Posisi di Rugi Laba</td>
                                    <td>
                                        <select class="form-control" name="cb_posisi_di_rugi_laba">
...
<script type="text/javascript">
    $(document).ready( function () {
        $('#table_data').DataTable({
            ...
            "retrieve" : true,
            "ajax": {
              "url" :  baseUrl + "/master_keuangan/kelompok_akun/tabel",
              "type": "GET"
            },
            "columns": [
            { "data": "kode" },
            { "data": "nama" },
            { "data": "posisi_di_buku_besar" },
            { "data": "posisi_di_rugi_laba" },
            { "data": "button" },
            ]
        });
    });
    ...
    $(document).on( "click",".btnedit", function() {
        ...
        $.ajax(
        {
            url : baseUrl + "/master_keuangan/kelompok_akun/get_data",
            type: "GET",
            data : value,
            dataType:'json',
            success: function(data, textStatus, jqXHR)
            {
                ...
                $("select[name='cb_posisi_di_buku_besar']").val(data.posisi_di_buku_besar);
                $("select[name='cb_posisi_di_rugi_laba']").val(data.posisi_di_rugi_laba);
                $("#modal").modal('show');
                $("input[name='ed_kode']").focus();
            },
    ...
    $(document).on("click","#btnsave",function(){
        $.ajax(
        {
            url : baseUrl + "/master_keuangan/kelompok_akun/save_data",
            type: "POST",
            dataType:"JSON",
    ...
    $(document).on( "click",".btndelete", function() {
        ...
        $.ajax({
            type: "POST",
            url : baseUrl + "/master_keuangan/kelompok_akun/hapus_data",
            dataType:"JSON",
            data: value,
            success: function(data, textStatus, jqXHR)
            {

                        ...
                        <form class="form-horizontal  kirim">
                          <table id="table_data" class="table table-striped table-bordered table-hover">
                            <tbody>
                                ...
                                <tr>
                                    <td style="padding-top: 0.4cm">Bahan Bakar</td>
                                    <td>
                                        <select class="form-control" class="bbm" name="bbm">
                                            <option value=""></option>
                                        @foreach ($bbm as $row)
                                            <option value="{{ $row->mb_id}}"> {{ $row->mb_nama}} </option>
                                        @endforeach
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    <td style="padding-top: 0.4cm">Km Per Liter</td>
                                    <td><input type="text" class="form-control" name="km" style="text-transform: uppercase" ></td>
                                </tr>
                            </tbody>
                          </table>
                        </form>
...
<script type="text/javascript">
    $(document).ready( function () {
        $('#table_data').DataTable({
            ...
            "retrieve" : true,
            "ajax": {
              "url" :  baseUrl + "/master_sales/tipe_angkutan/tabel",
              "type": "GET"
            },
            "columns": [
            { "data": "kode" },
    ...
    $(document).on( "click",".btnedit", function() {
        ...
        $.ajax(
        {
            url : baseUrl + "/master_sales/tipe_angkutan/get_data",
            type: "GET",
            data : value,
            dataType:'json',
            success: function(data, textStatus, jqXHR)
            {
                ...
                $("select[name='bbm']").val(data.bahan_bakar);
                $("input[name='km']").val(data.bbm_per_liter);
                $("#modal").modal('show');
                $("input[name='ed_kode']").focus();
            },
    ...
    $(document).on("click","#btnsave",function(){
        $.ajax(
        {
            url : baseUrl + "/master_sales/tipe_angkutan/save_data",
            type: "POST",
            dataType:"JSON",
    ...
    $(document).on( "click",".btndelete", function() {
        ...
        $.ajax({
            type: "POST",
            url : baseUrl + "/master_sales/tipe_angkutan/hapus_data",
            dataType:"JSON",
            data: value,
            success: function(data, textStatus, jqXHR)
            {

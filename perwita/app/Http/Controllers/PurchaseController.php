...
class PurchaseController extends Controller
{
	...
	public function spp_index () {
		...
		if(Auth::user()->punyaAkses('Surat Permintaan Pembelian','all')){
			...
			$data['disdetujui'] = DB::table("spp")->where('spp_status' , '=' , 'DISETUJUI')->where('spp_cabang' , '=' , $cabang)->count();
...
/*
	...
	*/	$data['spp'] = DB::select("select * from confirm_order, spp, spp_totalbiaya, supplier, masterdepartment where spp_bagian = kode_department and co_idspp = spp_id and spptb_idspp = spp_id and spptb_supplier = idsup and co_id='$id'");
	...
	public function lihatjurnalpenerimaan($id){
	...
	/*dd($request);*/
		...
		//	$newtime = date('Y-M-d H:i:s', $time);  
			...
			if($request->updatestock[0] == 'T'  || $request->kodestock[0] == 'T'){
		        ...
		        $jurnal->jr_year = date('Y');
		        $jurnal->jr_date = date('Y-m-d');
		        $jurnal->jr_detail = 'FP ITEM';
		        $jurnal->jr_ref = $nofaktur;
		        $jurnal->jr_note = 'FP ITEM';
		        $jurnal->save();
...
public function kekata($x) {
	...
	public function nocheckpelunasanhutangbank(Request $request){
		...
		$datas['fpgbank'] = DB::select("select * from fpg_cekbank,fpg where fpgb_kodebank = '$idbank' and fpgb_idfpg = idfpg and fpg_posting = 'NOT' order by idfpg ASC");
		$datas['fpgbank1'] = DB::select("select * from fpg_cekbank,fpg where fpgb_kodebank = '$idbank' and fpgb_idfpg = idfpg and fpg_posting = 'NOT' order by idfpg ASC");
	...
	public function getcek(Request $request){
		...
		if($jenisbayar == '4'){
			...
			$data['fpg'] = DB::select("select * from fpg, fpg_cekbank, cabang, masterbank, jenisbayar where idfpg = '$idfpg' fpg_jenisbayar = idjenisbayar and fpg_idbank = mb_id and fpgb_id = '$idfpgb' and fpgb_idfpg = idfpg and idfpg = '$idfpg'");
		}
	}

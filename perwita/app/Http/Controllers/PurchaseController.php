		...
		for($j=0; $j < count($array); $j++){
			...
			if($flag[$j] == 'PO'){				
				if($jenis[$j] != 'J'){
					...
					$data['po'][] = DB::select("select po_id ,po_no, po_subtotal, po_tipe, po_ppn, po_jenisppn, po_totalharga  from pembelian_order where  po_no = '$no_po' and po_statusreturn = 'AKTIF'  Group by po_id, po_no");

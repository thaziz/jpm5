...
class entri_transaksiController extends Controller {
    ...
    public function index() {
        return view('entri_transaksi.indexs');
    }
    ...
    public function index_data(Request $request) {        
        ...
        $jurnal = d_jurnal::select(DB::raw('@rownum  := @rownum  + 1 AS rownum'), 'jr_id', 'jr_trans', 'jr_cashtype', 'jr_tgl', 'jr_detail', 'jr_value', 'jr_note')->
                        where('jr_comp', '=', $comp)->where('jr_year', '=', $year)->OrderBy('jr_tgl','DESC');
...
//        return view('entri_transaksi.index', compact('jurnal'));
    ...
    public function cari_tanggal_get(Request $request) {        
        ...
        $end_nominal=$request->end_nominal;
        $request->start_nominal=str_replace(['Rp', '\\','.',' '], '', $request->start_nominal);
        $request->end_nominal=str_replace(['Rp', '\\','.',' '], '', $request->end_nominal);
        if($pilihan==1){
          // dd($request->start_tgl);
            $jurnal = d_jurnal::select(DB::raw('@rownum  := @rownum  + 1 AS rownum'), 'jr_id', 'jr_trans', 'jr_cashtype', 'jr_tgl', 'jr_detail', 'jr_value', 'jr_note')->
                        where('jr_comp', '=', $comp)->where('jr_year', '=', $year)
                        ->whereBetween('jr_tgl', array($request->start_tgl,$request->end_tgl))
                        ->OrderBy('jr_tgl','DESC')->get();
            //dd($pilihan);            
            return view('entri_transaksi.cari',compact('jurnal','pilihan',
                'start_nominal','end_nominal'));
        }                        
        else if($pilihan==0){                
            $jurnal = d_jurnal::select(DB::raw('@rownum  := @rownum  + 1 AS rownum'), 'jr_id', 'jr_trans', 'jr_cashtype', 'jr_tgl', 'jr_detail', 'jr_value', 'jr_note')->
                        where('jr_comp', '=', $comp)->where('jr_year', '=', $year)
                        ->whereBetween('jr_value', array($request->start_nominal,$request->end_nominal))
                        ->OrderBy('jr_tgl','DESC')->get();                    

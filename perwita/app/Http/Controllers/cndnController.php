			...
			for($i = 0; $i < count($request->idfaktur); $i++){
				...
				$iddt = DB::table('cndnpembelian_dt')  	
					...
					if($request->jeniscndn == 'CN'){
						...
						for($i = 0; $i < count($datacndn); $i++){
							//$totalhutangarga = $request->qtyterima[$i] * $request->jumlahharga[$i];
							$idakun = $datacndn[$i]->cndn_acccndn;
							$cabang = $datacndn[$i]->cndn_comp;
							$dataakun = DB::select("select * from d_akun where id_akun LIKE '$idakun%' and kode_cabang = '$cabang'");

							$idakun = $dataakun[0]->id_akun;
							$datadka = $dataakun[0]->akun_dka;
							...
							$akunhutangdagang = $datacndn[0]->cndn_acchutangdagang;
							...
							if($dka == 'D'){
								$dataakun2 = array (
									'id_akun' => $datacndn[0]->cndn_acchutangdagang,
									'subtotal' => '-' . $bruto,
									'dk' => 'K',
									'detail' => $datacndn[0]->cndn_keterangan,
								);
							}else{
								$dataakun2 = array (
									'id_akun' => $datacndn[0]->cndn_acchutangdagang,
							...
							array_push($datajurnal, $dataakun2);

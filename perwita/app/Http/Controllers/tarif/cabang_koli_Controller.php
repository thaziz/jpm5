...
class cabang_koli_Controller extends Controller
{
    public function table_data () {
        $sql = "    SELECT t.kode, t.id_kota_asal, k.nama asal,t.id_kota_tujuan, kk.nama tujuan, t.harga, t.jenis, t.waktu, t.keterangan  
    ...
    public function get_data (Request $request) {
        $id =$request->input('id');
        $data = DB::table('tarif_cabang_koli')->where('kode', $id)->first();
    ...
    public function save_data (Request $request) {
        ...
        $data = array(
                ...
                'tarif_reguler' => $request->tarif_reguler,
                'estimasi_reguler' => $request->estimasi_reguler,
                'tarif_express' => $request->tarif_express,
                'estimasi_express' => $request->estimasi_express,
        ...
        if ($crud == 'N') {
            ...
            if ($data['kode'] ==''){
                ...
                $sql = "	SELECT CAST(MAX(SUBSTRING (kode FROM '....$')) AS INTEGER) + 1 nomor FROM tarif_cabang_koli ";
            ...
            // end auto number
            $simpan = DB::table('tarif_cabang_koli')->insert($data);
        }elseif ($crud == 'E') {
            $simpan = DB::table('tarif_cabang_koli')->where('kode', $request->ed_kode_old)->update($data);
        }
        if($simpan == TRUE){
            $result['error']='';
            $result['result']=1;
    ...
    public function hapus_data (Request $request) {
        $hapus='';
        $id=$request->id;
        $hapus = DB::table('tarif_cabang_koli')->where('kode' ,'=', $id)->delete();
        if($hapus == TRUE){
            $result['error']='';
            $result['result']=1;
        }else{
    ...
    public function index(){
        ...
        return view('tarif.cabang_koli.index',compact('kota','cabang','akun'));
    }

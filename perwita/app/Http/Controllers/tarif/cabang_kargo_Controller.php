...
class cabang_kargo_Controller extends Controller
{
    public function table_data () {
        $sql = "    SELECT t.kode, t.id_kota_asal, k.nama asal,t.id_kota_tujuan, kk.nama tujuan, t.harga, t.jenis, t.waktu, t.kode_angkutan, a.nama angkutan 
    ...
    public function save_data (Request $request) {
        ...
        if ($crud == 'N') {
            ...
            if ($data['kode'] ==''){
                ...
                $sql = "	SELECT CAST(MAX(SUBSTRING (kode FROM '....$')) AS INTEGER) + 1 nomor
                            FROM tarif_cabang_kargo WHERE kode_cabang='$kode_cabang'
                            AND kode LIKE '%".$kode_cabang."%' ";
                $list = collect(\DB::select($sql))->first();
                if ($list->nomor == ''){
                    //$data['nomor']='SJT-'.$kode_cabang.'-'.$tanggal.'-00001';
                    $data['kode']=$kode_cabang.'00001';
                } else{
                    $kode  = substr_replace('00000',$list->nomor,-strlen($list->nomor)); 
                    $data['kode']=$kode_cabang.$kode;
                }
            }
            // end auto number
            $simpan = DB::table('tarif_cabang_kargo')->insert($data);
        }elseif ($crud == 'E') {
            $simpan = DB::table('tarif_cabang_kargo')->where('kode', $request->ed_kode_old)->update($data);
        }
        if($simpan == TRUE){
            $result['error']='';
            $result['result']=1;
    ...
    public function index(){
        ...
        return view('tarif.cabang_kargo.index',compact('kota','angkutan','cabang','satuan','akun'));
    }

...
class cabang_kilogram_Controller extends Controller
{
    ...
    public function get_data (Request $request) {
        $id =$request->input('id');
        $data = DB::table('tarif_cabang_kilogram')->where('kode', $id)->first();
    ...
    public function save_data (Request $request) {
        ...
        if ($crud == 'N') {
            ...
            if ($data['kode'] ==''){
                ...
                $sql = "	SELECT CAST(MAX(SUBSTRING (kode FROM '....$')) AS INTEGER) + 1 nomor FROM tarif_cabang_kilogram ";
            ...
            // end auto number
            $simpan = DB::table('tarif_cabang_kilogram')->insert($data);
        }elseif ($crud == 'E') {
            $simpan = DB::table('tarif_cabang_kilogram')->where('kode', $request->ed_kode_old)->update($data);
        }
        if($simpan == TRUE){
            $result['error']='';
            $result['result']=1;
    ...
    public function hapus_data (Request $request) {
        $hapus='';
        $id=$request->id;
        $hapus = DB::table('tarif_cabang_kilogram')->where('kode' ,'=', $id)->delete();
        if($hapus == TRUE){
            $result['error']='';
            $result['result']=1;
        }else{

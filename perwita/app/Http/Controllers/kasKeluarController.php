	...
	public function datatable_bkk(request $req)
	{
		...
		if ($req->nota != '0') {
			...
			if (Auth::user()->punyaAkses('Bukti Kas Keluar','all')) {
				...
				$cabang = Auth::user()->kode_cabang;
				$sql = "SELECT * FROM bukti_kas_keluar JOIN jenisbayar on idjenisbayar = bkk_jenisbayar join cabang on kode = bkk_comp where bkk_nota != '0' bkk_comp = '$cabang' $tanggal_awal $tanggal_akhir $jenis_biaya";
				$data = DB::select($sql);
			}
		}
        $data = collect($data);

...
//	dd($setujumankeu);
	...
	/*	dd($data['spp']);*/
		...
		if($tipespp != 'J'){
			...
			if($tipespp == 'NS' && $data['jenisitem'] == 'S'){
				$data['sppdt_barang'] = DB::select("select distinct sppd_kendaraan , sppd_kodeitem, nama_masteritem, sppd_qtyrequest, sg_qty, unitstock from kendaraan, masteritem , spp_detail LEFT OUTER JOIN stock_gudang on sppd_kodeitem = sg_item and sg_gudang = '$lokasigudang' where sppd_idspp = '$id' and kode_item = sppd_kodeitem order and sppd_kendaraan = kendaraan.id by sppd_kodeitem asc");
...
public function purchase_order() {
	...
	public function ajax_tampilspp(Request $request){
		...
		for($j=0; $j < count($array); $j++){
			...
			$data['codt'][] = DB::select("select * from confirm_order, confirm_order_dt , confirm_order_tb, spp, masteritem LEFT OUTER JOIN itemsupplier on kode_item = is_kodeitem where co_idspp = '$idspp' and codt_idco = co_id and cotb_idco = co_id and co_idspp = spp_id and codt_supplier = cotb_supplier and codt_supplier = '$nosupplier' and codt_kodeitem = kode_item and cotb_id = '$idcotb' and co_id = '$idco' ");
